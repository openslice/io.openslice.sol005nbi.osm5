package osm5.ns.yang.nfvo.mano.types.rev170208.http.endpoints.http.endpoint;
...
/**
 * Class that builds {@link HeadersBuilder} instances.
 * 
 * @see HeadersBuilder
 *
 */
public class HeadersBuilder implements Builder<Headers> {
    ...
    public HeadersBuilder(Headers base) {
        ...
        if (base instanceof HeadersImpl) {
            ...
            Map<Class<? extends Augmentation<Headers>>, Augmentation<Headers>> aug =((AugmentationHolder<Headers>) base).augmentations();
            if (!aug.isEmpty()) {
    ...
    private static final class HeadersImpl implements Headers {
        ...
        HeadersImpl(HeadersBuilder base) {
            if (base.key() != null) {
                this.key = base.key();
            } else {
                this.key = new HeadersKey(base.getKey());
            }
