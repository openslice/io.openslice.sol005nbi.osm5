...
import osm5.ns.yang.nfvo.nsr.rev170208.exec.ns.service.primitive.input.vnf.list.VnfPrimitive;
...
/**
 * Class that builds {@link VnfListBuilder} instances.
 * 
 * @see VnfListBuilder
 *
 */
public class VnfListBuilder implements Builder<VnfList> {
    ...
    public VnfListBuilder(VnfList base) {
        ...
        if (base instanceof VnfListImpl) {
            ...
            Map<Class<? extends Augmentation<VnfList>>, Augmentation<VnfList>> aug =((AugmentationHolder<VnfList>) base).augmentations();
            if (!aug.isEmpty()) {
    ...
    private static final class VnfListImpl implements VnfList {
        ...
        VnfListImpl(VnfListBuilder base) {
            if (base.key() != null) {
                this.key = base.key();
            } else {
                this.key = new VnfListKey(base.getMemberVnfIndexRef());
            }
