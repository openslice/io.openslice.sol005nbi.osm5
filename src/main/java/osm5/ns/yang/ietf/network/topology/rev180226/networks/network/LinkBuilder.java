...
/**
 * Class that builds {@link LinkBuilder} instances.
 * 
 * @see LinkBuilder
 *
 */
public class LinkBuilder implements Builder<Link> {
    ...
    public LinkBuilder(Link base) {
        ...
        if (base instanceof LinkImpl) {
            ...
            Map<Class<? extends Augmentation<Link>>, Augmentation<Link>> aug =((AugmentationHolder<Link>) base).augmentations();
            if (!aug.isEmpty()) {
    ...
    private static final class LinkImpl implements Link {
        ...
        LinkImpl(LinkBuilder base) {
            if (base.key() != null) {
                ...
                this.key = new LinkKey(base.getLinkId());
            }
